...
def dct(x, norm=None):
    ...
    """
    x_shape = x.shape  # 32,15,512
    N = x_shape[-1]
    x = x.contiguous().view(-1, N) # 480,512

    v = torch.cat([x[:, ::2], x[:, 1::2].flip([1])], dim=1)  # 480,512

    Vc = dct_fft_impl(v)  # 480,512,2

    k = - torch.arange(N, dtype=x.dtype, device=x.device)[None, :] * np.pi / (2 * N)  # 1, 512
    W_r = torch.cos(k)
    W_i = torch.sin(k)

    V = Vc[:, :, 0] * W_r - Vc[:, :, 1] * W_i  # 480, 512

    if norm == 'ortho':
        V[:, 0] /= np.sqrt(N) * 2
        V[:, 1:] /= np.sqrt(N / 2) * 2

    V = 2 * V.view(*x_shape)  # 32,15,512
