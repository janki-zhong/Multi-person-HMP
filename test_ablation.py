...
class Tester:
    def __init__(self, args):
        ...
        test_dataset = Data(dataset='mupots', mode=0, device=args.device, transform=False)  # mocap_umpm mupots
        print("Load Test set!")
        self.test_dataloader = DataLoader(test_dataset, batch_size=16, shuffle=False, drop_last=True)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            print(
                "{0: <16} | {1:6d} | {2:6d} | {3:6d} | {4:6d} | {5:6d}| {6: <16}".format("Lengths", 200, 400, 600, 800, 1000, "average"))
            print("=== JPE Test Error ===")
            jpe_average = (jpe_err_total[0] / n + jpe_err_total[2] / n + jpe_err_total[4] / n) / 3
            print(
                "{0: <16} | {1:6.0f} | {2:6.0f} | {3:6.0f} | {4:6.0f} | {5:6.0f} | {6:6.0f}".format("Our",
                                                                                                    jpe_err_total[0] / n,
                                                                                                    jpe_err_total[1] / n,
                                                                                                    jpe_err_total[2] / n,
                                                                                                    jpe_err_total[3] / n,
